...
public class AdaptiveConcessionStrategy {
	 ...
	 */
	public void FindConcessionPoint(int numberOfRows, int numberOfColumns, DetectionRegion detReg, double reservePrice, Date deadline, long stepSize, int numberOfRounds, ArrayList<Offer> offerHistory, Offer prevOffer, boolean isBuyer){
			...
			for(int i = 0; i < numberOfRows; i++){
				...
				for(int j = 0; j < numberOfColumns; j++){
					...
					if(!detReg.getCells()[i][j].isExpired()){
						...
						this.opponentOfferAtTb = calculator.GenerateFittedOfferForGivenTime(detReg, i, j, offerHistory, numberOfRounds, deadline);
						this.opponentOfferAtT0 = calculator.GenerateFittedOfferForGivenTime(detReg, i, j, offerHistory, numberOfRounds, prevOffer.getOfferTime());
						...
						if(isBuyer){
							this.buyerConcessionStrategyForGivenCell(i, j, detReg, reservePrice, deadline, stepSize, numberOfRounds, offerHistory);
						}
						else{
							this.sellerConcessionStrategyForGivenCell(i, j, detReg, reservePrice, deadline, stepSize, numberOfRounds, offerHistory);
						}
	 ...
	 */
	public void buyerConcessionStrategyForGivenCell(int row, int col, DetectionRegion detReg, double reservePrice, Date deadline, long stepSize, int numberOfRounds, ArrayList<Offer> offerHistory){
		
		System.out.println("\n=======================this is the buyer concession strategy for the given cell method=================\n");
		...
		//concession strategy scenario : 2
			...
			//case 2: line l1 - regression line which travels across the negotiation region
				...
				this.concessionPrice = calculator.GenerateFittedOfferForGivenTime(detReg, row, col, offerHistory, numberOfRounds, concessionTime);
			} //end of case: 2
		...
		//concession strategy scenario : 3
			...
			if( opponentOfferAtT0 > p0){
				...
				Date timeAtOfferP0 = calculator.GeneratedTimeForGivenFittedOffer(detReg, row, col, offerHistory, numberOfRounds, p0);
		...
		//concession strategy scenario : 4
			...
			//case 4: line l2- regression line which travels across the negotiation region and goes underneath the p0 line at the dead line
				...
				//calculate the time at the offer p0
				this.timeAtOfferP0 = calculator.GeneratedTimeForGivenFittedOffer(detReg, row, col, offerHistory, numberOfRounds, p0);
	 ...
	 */
	public void sellerConcessionStrategyForGivenCell(int row, int col, DetectionRegion detReg, double reservePrice, Date deadline, long stepSize, int numberOfRounds, ArrayList<Offer> offerHistory){
